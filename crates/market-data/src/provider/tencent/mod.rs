...
impl TencentProvider {
    ...
    fn price_scale_for_symbol(symbol: &str) -> Decimal {
        // Eastmoney's realtime integer fields use three decimals for HK
        // securities and two decimals for mainland securities. Historical
        // K-lines are already returned as decimal strings.
        if symbol.starts_with("116.") {
            Decimal::from(1000)
        } else {
            Decimal::from(100)
    ...
    fn parse_quote(symbol: &str, body: QuoteResponse) -> Result<Quote, MarketDataError> {
        if body.rc != 0 {
            return Err(Self::provider_error(format!("Quote API error {}", body.rc)));
        }
        let data = body
            .data
            .ok_or_else(|| MarketDataError::SymbolNotFound(symbol.to_string()))?;
        let scale = Self::price_scale_for_symbol(symbol);
        let scaled = |value: Option<i64>| value.map(|v| Decimal::from(v) / scale);
        let close = scaled(data.close)
            .ok_or_else(|| MarketDataError::SymbolNotFound(symbol.to_string()))?;
        let timestamp = data
            .timestamp
        ...
        Ok(Quote {
            timestamp,
            open: scaled(data.open),
            high: scaled(data.high),
            low: scaled(data.low),
            close,
            volume: data.volume.map(Decimal::from),
    ...
    async fn fetch_kline(
        ...
        let response = self
            .checked_response(self.client.get(KLINE_URL).query(&[
                ("secid", symbol),
                ("klt", "101"),
                ("fqt", "1"),
                ("beg", begin.as_str()),
        ...
        let body: KlineResponse = response
            .json()
            .await
            .map_err(|e| Self::provider_error(format!("Failed to parse kline response: {}", e)))?;
        if body.rc != 0 {
            return Err(Self::provider_error(format!(
                "Kline API error {}: {}",
                body.rc, body.message
            )));
        }
        let rows = body
            .data
        ...
        for raw in rows {
            ...
            let date = NaiveDate::parse_from_str(row[0], "%Y-%m-%d").map_err(|_| {
                MarketDataError::ValidationFailed {
                    message: format!("Invalid kline date '{}'", row[0]),
                }
            })?;
            ...
            let close = parse(row[2]).ok_or_else(|| MarketDataError::ValidationFailed {
                message: format!("Invalid kline close '{}'", row[2]),
            })?;
            let timestamp = date
                .and_hms_opt(12, 0, 0)
    ...
    async fn fetch_fx_quotes(
        ...
        let body: KlineResponse = response.json().await.map_err(|e| {
            Self::provider_error(format!("Failed to parse Eastmoney FX response: {}", e))
        })?;
        if body.rc != 0 {
            return Err(Self::provider_error(format!(
                "Eastmoney FX API error {}: {}",
                body.rc, body.message
            )));
        }
        let rows = body
        ...
        for raw in rows {
            ...
            let date = NaiveDate::parse_from_str(row[0], "%Y-%m-%d").map_err(|_| {
                MarketDataError::ValidationFailed {
                    message: format!("Invalid FX date '{}'", row[0]),
                }
            })?;
            if date < start || date > end {
                continue;
            ...
            let parse = |value: &str| -> Option<Decimal> { value.parse().ok() };
            let close = parse(row[2]).ok_or_else(|| MarketDataError::ValidationFailed {
                message: format!("Invalid FX close '{}'", row[2]),
            })?;
            let timestamp = date
                .and_hms_opt(12, 0, 0)
                .map(|dt| Utc.from_utc_datetime(&dt))
...
impl MarketDataProvider for TencentProvider {
    ...
    async fn get_latest_quote(
        ...
        let response = self
            .checked_response(self.client.get(QUOTE_URL).query(&[
                ("secid", symbol.as_str()),
                ("fields", "f43,f44,f45,f46,f47,f57,f58,f60,f86"),
            ]))
            .await?;
        let body = response
            .json()
            .await
            .map_err(|e| Self::provider_error(format!("Failed to parse quote response: {}", e)))?;
    ...
    async fn get_historical_quotes(
        ...
        if let ProviderInstrument::FxPair { from, to } = &instrument {
            return self
                .fetch_fx_quotes(from, to, start.date_naive(), end.date_naive())
                .await;
        }
        let symbol = Self::extract_symbol(&instrument)?;
    ...
    async fn search(&self, query: &str) -> Result<Vec<SearchResult>, MarketDataError> {
        let response = self
            .checked_response(self.client.get(SEARCH_URL).query(&[
                ("input", query),
                ("type", "14"),
                ("token", SEARCH_TOKEN),
                ("count", "20"),
        ...
        for item in body.table.data {
            let (mic, currency, exchange) = match item.market.as_str() {
                "1" => ("XSHG", "CNY", "SH"),
                "0" => ("XSHE", "CNY", "SZ"),
                "116" => ("XHKG", "HKD", "HK"),
                _ => continue,
            ...
            results.push(
                SearchResult::new(&item.code, &item.name, exchange, "EQUITY")
...
struct SearchResponse {
    #[serde(default, rename = "QuotationCodeTable")]
    table: SearchTable,
}
#[derive(Debug, Default, Deserialize)]
struct SearchTable {
...
mod tests {
    ...
    #[test]
    fn provider_id_remains_compatible() {
        assert_eq!(TencentProvider::new().id(), "TENCENT");
    }
    #[test]
    fn normalizes_legacy_and_current_symbols() {
    ...
    fn parses_a_share_quote() {
        ...
        assert_eq!(quote.volume, Some(dec!(123456)));
        assert_eq!(quote.currency, "CNY");
    }

    #[test]
    fn parses_hk_quote_with_three_decimal_scale() {
        let body = QuoteResponse {
            rc: 0,
            data: Some(QuoteData {
                close: Some(440_000),
        ...
        let quote = TencentProvider::parse_quote("116.00700", body).unwrap();
        assert_eq!(quote.close, dec!(440));
        assert_eq!(quote.open, Some(dec!(441)));
        assert_eq!(quote.currency, "HKD");
    }
    #[test]
    fn supports_cn_and_hk() {
        let provider = TencentProvider::new();
        for (ticker, mic) in [("600000", "XSHG"), ("000001", "XSHE"), ("00700", "XHKG")] {
            let instrument = crate::models::InstrumentId::Equity {
